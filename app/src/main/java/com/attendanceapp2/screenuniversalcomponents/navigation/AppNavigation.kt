...
//    val screenViewModel: ScreenViewModel = viewModel(factory = AppViewModelProvider.Factory)
    ...
    NavHost(
        ...
        navigation(
            route = AppRoutes.PORTALS.name,
            startDestination = when(signInViewModel.checkUser().lowercase()){
                "student" -> AppRoutes.STUDENT.name
                "admin" -> AppRoutes.ADMIN.name
                "faculty" -> AppRoutes.FACULTY.name
                else -> "test"
